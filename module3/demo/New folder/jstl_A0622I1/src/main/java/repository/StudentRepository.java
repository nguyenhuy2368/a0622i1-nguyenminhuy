...
public class StudentRepository implements IStudentRepository{
    ...
    private final String SEARCH = "select s.*, c.name as class_name from student s join clazz c on s.class_id=c.id where s.name like ? and s.point like ? and s.class_id like ?";
    ...
    @Override
    public List<Student> search(String nameSearch, String pointSearch, String classIdSearch) {
        List<Student> studentList = new ArrayList<>();
        Connection  connection = BaseRepository.getConnectDB();
        // tao câu truy vấn
        try {
            ...
            preparedStatement.setString(2,"%"+pointSearch+"%");
            preparedStatement.setString(3,"%"+classIdSearch+"%");
            ResultSet resultSet =preparedStatement.executeQuery();
            while (resultSet.next()){
                int id = resultSet.getInt("id");
                String name = resultSet.getString("name");
